<template>
  <p class="title">{{ $t('homepage.lemonaidea_text_improve') }}</p>

  <div class="main-box">
    <div class="title-box">
      <img src="@/assets/line.png" alt="" />
      <span>{{ $t('fun_content_opt.lemonaidea_text_improve_edit_tips_a') }}</span>
    </div>
    <nut-textarea v-model="title" :rows="3" autosize :placeholder="$t('fun_content_opt.lemonaidea_text_improve_edit_tips_b')" />
...
<script lang="ts" setup>
  ...
  const handleReSend = () => {
    ...
    generate(objEle, lang.value)
      ...
      .finally(() => {})
      .catch((err: any) => showToast(t('all.lemonaidea_toast_fail')));
    title.value = '';
    subject.value = '';
  };
  const handleSend = () => {
